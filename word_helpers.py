...
# Make POS tags more user-friendly to print to the console in feedback
pos_map = {
...
def adjective_func(word1, word2, tag1, tag2):
  if tag1 == "adjective" and tag2 == "adjective":
    ...
    if ((word1[-1] == "a" or word1[-2:] == "as") and (word2[-1] != "a" or word2[-2:] != "as")) or ((word2[-1] == "a" or word2[-2:] == "as") and (word1[-1] != "a" or word1[-2:] != "as")):
...
# Minimum Edit Distance (MED) for hints based on POS
# (NOTE: a correct POS but wrong word will not affect the edit cost at all. POS tags are included as a simple
